  ...
  return (
    <div className='w-full mx-auto max-w-7xl p-2 px-4 relative aspect-16/7 mb-5 group'>
      <Link href={`/blog/${slides[currentIndex]?.id}`}>
        <div style={{ backgroundImage: `url(${process.env.NEXT_PUBLIC_IMAGE_SERVICE}${slides[currentIndex]?.image})` }} className='relative flex items-center justify-center bg-cover w-full h-full rounded-xl bg-center ng-cover duration-500'>
